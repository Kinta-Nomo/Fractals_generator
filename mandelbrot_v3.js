...
function drawMandelbrot(x, y){
  for (var i = -range; i < range; i++){
    for (var j = -range; j < range; j++){
      colour = mandelbrot(x + (i)/zoom, y + (j)/zoom, 0, 0, 50)
  ...
  if (difference[0] > 0){
    ...
    for (i = 0; i<difference[0]; i++){
      ...
      for (j = -range; j<range; j++){
        picture[i].push(mandelbrot(centerx + (i-200)/zoom, centery + (j)/zoom, 0, 0, iteration))
      }
    }
    ...
    for (i = 0; i>difference[0]; i--){
      ...
      for (j = -range; j<range; j++){
        picture[picture.length-1+i].push(mandelbrot(centerx + (i+200)/zoom, centery + (j)/zoom, 0, 0, iteration))
      }
    }
  }
